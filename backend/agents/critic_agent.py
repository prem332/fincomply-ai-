...
def _check_circular_freshness(published_date_str: Optional[str]) -> dict:
    ...
    try:
        ...
        if age_days <= CIRCULAR_FRESHNESS_DAYS:
            return {"status": "PASS", "note": f"Circular is {age_days} days old (within {CIRCULAR_FRESHNESS_DAYS}-day threshold)"}
        else:
            return {
                "status": "FLAG",
                "note": f"Circular is {age_days} days old — exceeds {CIRCULAR_FRESHNESS_DAYS}-day freshness threshold. Check for newer version."
            }
    except Exception:
...
def _check_circular_number(circular_number: Optional[str]) -> bool:
    ...
    return lower not in ("unknown", "n/a", "none", "", "null")
...
def run_critic_agent(
    ...
    # If URL fails rule check, reject immediately (no LLM call needed)
    if not url_ok:
        return {
            ...
            "revision_instructions": "Re-fetch from official .gov.in source only. Reject all non-government sources.",
        }

    # ── LLM semantic evaluation ───────────────────────────────────────────────
    # Format source documents for critic to review
    ...
    source_docs_text = "\n\n".join(
        ...
        for i, s in enumerate(all_sources[:8])  # Limit to 8 sources for token budget
    ) or "No source documents provided."

    # Clean the answer dict before sending to LLM (remove internal keys)
    answer_for_critic = {
    ...
    try:
        ...
        # Return a conservative critique on failure
        return {
            "factual_accuracy": "FAIL",
            "source_verified": "FAIL" if not url_ok else "PASS",
            "recency": recency["status"],
            "recency_note": recency["note"],
            "completeness": "GAPS_FOUND",
            "gaps": [f"Critic Agent API error: {str(e)}"],
            "clarity": "FAIL",
            "actionability": "FAIL",
            "overall_verdict": "REVISE",
            "revision_instructions": "Critic Agent failed — Supervisor should apply conservative confidence.",
        }
